...
After the script is run, the `index.html` file can be opened. Here you need to make sure to open the file via some kind of HTTPRequest, not as a local file, because of CORS issues. Opening as localhost or via a local server (e.g. with the [VSCode Live Server Extension](https://marketplace.visualstudio.com/items?itemName=ritwickdey.LiveServer)) works. Then the visualization loads for a while. If it takes over 30sec, make sure to look into your browsers debug console(usually opened by pressing F12), to see if any errors occured.